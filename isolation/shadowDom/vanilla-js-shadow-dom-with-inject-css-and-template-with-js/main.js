...
async function loadExternalStyles(url) {
    const response = await fetch(url);
    const cssText = await response.text(); // тут сидит css в формате строки
...
class WrapperClass extends HTMLElement {
    async connectedCallback() {
        ...
        const templateContent = temp.content.textContent.trim(); 
        console.log(templateContent)

        this.attachShadow({ mode: 'open' });
        this.shadowRoot.innerHTML = this.render();
this.shadowRoot.innerHTML += `<link href="./light.css" rel="stylesheet"></link>`
        try {
        ...
        // Подключаем стили
        // const cssStr = await loadExternalStyles('./light.css')
...
customElements.define("wrapper-element", WrapperClass)

//Не использовать new CSSStyleSheet(); использовать <link></link>